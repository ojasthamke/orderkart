      ...
      for (int a = 0; a < 10; a++) {
        ...
        for (int s = 0; s < 5; s++) {
          ...
          for (int c = 0; c < 10; c++) {
            ...
            final houseNo =
                '#${_random.nextInt(300) + 101}, Block ${_random.nextInt(8) + 1}';

            final bool isVip = vipCount < 200;
            final String vipPlan =
                isVip ? vipPlans[vipCount % vipPlans.length] : '';
            ...
            await txn.insert(
                'customers',
                {
                  ...
                  'notes': isVip
                      ? 'VIP Customer Subscription Active'
                      : 'Regular Customer',
                  'maps_location': '',
                  'photo_path': '',
                  'serial_no': customerIndex,
                  'outstanding_balance': (customerIndex % 7 == 0)
                      ? (_random.nextInt(500) + 50).toDouble()
                      : 0.0,
            ...
            if (isVip) {
              await txn.insert(
                  'vip_membership',
                  {
                    ...
                    'fee': vipPlan == 'Diamond VIP'
                        ? 1499.0
                        : (vipPlan == 'Platinum VIP' ? 999.0 : 499.0),
                    'discount_pct': vipPlan == 'Diamond VIP'
                        ? 15.0
                        : (vipPlan == 'Platinum VIP' ? 10.0 : 5.0),
                    'markup_pct': 0.0,
                    'free_delivery': 1,
                    'priority_delivery': 1,
                    'status': 'active',
