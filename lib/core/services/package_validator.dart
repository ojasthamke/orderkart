  ...
  static Future<PackageValidationResult> validatePackage(String zipPath) async {
    try {
      ...
      if (secretKey.isEmpty && !isWorkerProvisioning && generatedByWorkerId.isEmpty) {
        ...
        final inlineSecret = manifest['owner_secret']?.toString() ?? '';
        if (inlineSecret.isNotEmpty) {
          ...
          // Fresh reinstall with no recoverable key — allow restore, skip HMAC
          // (the package was generated by this same owner app, trust is implicit)
          secretKey = '';
        }
      }

      // Validate HMAC signature (skip if key is unavailable — fresh reinstall scenario)
