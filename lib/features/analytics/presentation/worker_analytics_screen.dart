    ...
    final cashRes = await db.rawQuery('''
      ...
      WHERE (o.assigned_worker_id = ? OR o.created_by = ? OR o.worker_id = ? OR 
             c.assigned_worker_id = ? OR o.id IN (
               SELECT entity_id FROM worker_assignments WHERE worker_id = ? AND entity_type = 'order'
             )) AND LOWER(p.method) = 'cash' AND (o.delivery_status IS NULL OR (o.delivery_status != 'cancelled' AND o.delivery_status != 'denied'))
    ''', [wid, wid, wid, wid, wid]);
...
final singleWorkerStatsProvider =
  ...
  final salesRes = await db.rawQuery('''
    ...
    FROM orders 
    WHERE (assigned_worker_id = ? OR created_by = ?) AND (delivery_status IS NULL OR (delivery_status != 'cancelled' AND delivery_status != 'denied'))
  ''', [workerId, workerId]);
  ...
  final todaySalesRes = await db.rawQuery('''
    ...
    FROM orders 
    WHERE DATE(created_at) = DATE(?) AND (assigned_worker_id = ? OR created_by = ?) AND (delivery_status IS NULL OR (delivery_status != 'cancelled' AND delivery_status != 'denied'))
  ...
  final monthlySalesRes = await db.rawQuery('''
    SELECT COALESCE(SUM(grand_total), 0) as total_sales 
    FROM orders 
    WHERE strftime('%Y-%m', created_at) = ? AND (assigned_worker_id = ? OR created_by = ?) AND (delivery_status IS NULL OR (delivery_status != 'cancelled' AND delivery_status != 'denied'))
  ...
  final duesRes = await db.rawQuery('''
    ...
    FROM orders 
    WHERE remaining_amount > 0 AND (assigned_worker_id = ? OR created_by = ?) AND (delivery_status IS NULL OR (delivery_status != 'cancelled' AND delivery_status != 'denied'))
  ''', [workerId, workerId]);
  final pendingDues =
  ...
  final cashRes = await db.rawQuery('''
    ...
    FROM payments p
    JOIN orders o ON p.order_id = o.id
    WHERE (o.assigned_worker_id = ? OR o.created_by = ?) AND LOWER(p.method) = 'cash' AND (o.delivery_status IS NULL OR (o.delivery_status != 'cancelled' AND o.delivery_status != 'denied'))
  ''', [workerId, workerId]);
  final cashColl = (cashRes.first['sum'] as num?)?.toDouble() ?? 0.0;
  ...
  final weeklySales = await db.rawQuery('''
    ...
    WHERE created_at >= datetime('now', '-7 days') 
      ...
      AND (delivery_status IS NULL OR (delivery_status != 'cancelled' AND delivery_status != 'denied'))
