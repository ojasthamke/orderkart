...
class SettingsDao {
  ...
  Future<AppSettings> loadAllSettings() async {
    ...
    return AppSettings(
      ...
      deliveryCharge:
          double.tryParse(m[AppConstants.keyDeliveryCharge] ?? '10') ?? 10,
      ...
      notificationsEnabled:
          (m[AppConstants.keyNotifications] ?? 'true') == 'true',
      ...
      lastDeliveryCharge:
          double.tryParse(m[AppConstants.keyLastDeliveryCharge] ?? '10') ?? 10,
      notificationTime: m[AppConstants.keyNotifTime] ?? '06:00',
      enableVipPriceMarkup:
          (m[AppConstants.keyEnableVipPriceMarkup] ?? 'true') == 'true',
      language: m[AppConstants.keyLanguage] ?? 'en',
      workerDiscountCap:
          double.tryParse(m[AppConstants.keyWorkerDiscountCap] ?? '10') ?? 10.0,
      notificationSound: (m[AppConstants.keyNotifSound] ?? 'true') == 'true',
      notificationVibration:
          (m[AppConstants.keyNotifVibration] ?? 'true') == 'true',
      storeOpen: (m['store_open'] ?? 'true') == 'true',
      storeSchedule: m['store_schedule'] ?? '',
      enableDeliveryCharges:
          (m[AppConstants.keyEnableDeliveryCharges] ?? 'true') == 'true',
