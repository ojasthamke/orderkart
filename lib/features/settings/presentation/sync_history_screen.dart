...
final importHistoryProvider =
  ...
  return await db.query('import_history',
      orderBy: 'imported_at DESC', limit: 100);
});

final exportHistoryProvider =
    FutureProvider<List<Map<String, dynamic>>>((ref) async {
  final db = await DatabaseHelper.instance.database;
  return await db.query('export_history',
      orderBy: 'exported_at DESC', limit: 100);
});

final workerSyncHistoryProvider =
    FutureProvider<List<Map<String, dynamic>>>((ref) async {
  final db = await DatabaseHelper.instance.database;
      ...
      data: (logs) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (ctx, idx) {
            ...
            final deviceName =
                log['device_name'] as String? ?? 'Unknown Device';
      ...
      data: (logs) {
        if (logs.isEmpty) {
          return const EmptyStateWidget(
            ...
            subtitle:
                'All modular database packages you export are recorded here.',
          );
        }
        return ListView.builder(
          padding: const EdgeInsets.all(16),
  ...
  Widget _buildWorkerSyncList(WidgetRef ref) {
    ...
    return historyAsync.when(
      loading: () => const Center(
          child: CircularProgressIndicator(color: AppColors.primary)),
      error: (err, _) =>
          Center(child: Text('Error loading worker syncs: $err')),
      data: (logs) {
        if (logs.isEmpty) {
          return const EmptyStateWidget(
